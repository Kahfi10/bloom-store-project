...
// BUG-11 fix: NO fallback defaults — fail loudly if env vars are missing
...
export async function POST(req: NextRequest) {
  // BUG-09 fix: wrap entire handler in try-catch so DB errors return proper JSON
  try {
    ...
    // BUG-11 fix: guard against missing env vars
    if (!ADMIN_USERNAME || !ADMIN_PASSWORD || !ADMIN_ACCESS_CODE || !ADMIN_SECRET_KEY) {
      console.error('Admin credentials env vars not configured.');
    ...
    try {
      await prisma.adminLoginAttempt.create({ data: { ip, success: credentialsOk } });
    } catch { /* non-critical */ }
